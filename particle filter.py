# -*- coding: utf-8 -*-
"""
Created on Tue Sep 05 15:55:04 2017
...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # ADD CODE HERE
        length = 20.
        global bearing_noise,steering_noise,distance_noise
    
        d = motion[1]
        beta = tan(motion[0])*d/length
        theta = self.orientation
        if abs(beta) < 0.001:
            x = self.x + d*cos(theta)
            y = self.y + d*sin(theta)
            orie = self.orientation
        else:
            R = d/beta
    ...
    def sense(self,noise): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            theta_glob = atan2(landmarks[i][0]-self.y,landmarks[i][1]-self.x)
            theta = (theta_glob-self.orientation)%(2*pi)
            Z.append(theta)
        global bearing_noise,steering_noise,distance_noise
        self.set_noise(bearing_noise,steering_noise,distance_noise)
        if noise == 0:
            self.set_noise(0.,0.,0.)
