...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        #
        #            
        center1 = [radius,radius] # x,y
        center2 = [3*radius,radius]
        x = self.x
        y = self.y
        
        if x <= 3*radius and x >= radius:
            if y > radius:
                cte = y - 2*radius
            else:
                cte = -y
        elif x > 3*radius:
            cte = ((y - center2[1])**2 + (x-center2[0])**2)**0.5-radius
        else:
            cte = ((y - center1[1])**2 + (x-center1[0])**2)**0.5-radius
